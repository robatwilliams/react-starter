const { mapKeys, merge, uniq } = require('lodash');
...
HtmlModuleScriptWebpackPlugin.prototype = {
  ...
  beforeHtmlProcessing: function (htmlPluginData, callback) {
    ...
    // Keep modern/legacy pairs together, to aid sorting shared chunks immediately after the
    // manifest that gets loaded - whichever one that is.
    const allJs = [];
    htmlPluginData.assets.js.forEach((newJs, index) => {
      const existingJs = this.sharedAssets.js[index];
      if (existingJs) {
        allJs.push(existingJs);
      }
